...
<section class="ui black segment container">
	<div class="ui grid">
		<div class="twelve wide column">
			...
			<h3 style="display:block;">${league.season}/${f:substring(league.season+1, 2, 4)} ${!empty league.round ? "Season" + league.round + "Round" : "Free Season"}</h3>
		</div>
		<div class="four wide column" style="display: flex; align-items: center;">
			<input class="ui fluid black button f k r opening" id="${!empty league.round ? 'progress' : 'opening'}" type="button" value="${!empty league.round ? '진행' : '개막'}" style="height:75%;">
		</div>
	</div>
	<hr>
	<c:if test="${empty league.round}">
		...
		<div class="ui transition hidden center aligned black segment" id="openingSection">
			...
			<div class="ui centered grid segment">
				<c:forEach var="club" items="${league.roster}">
					<div class="column">
			...
			<div class="ui container f k r">
				총 ${f:length(league.roster)} 개의 팀이 ${(f:length(league.roster)-1)*2} round 동안 경기를 치루게 됩니다. 
			</div>
