		...
		<div class="ui segment f k r">
			...
			<c:if test="${fifa.role ne 'Player'}">
				<div class="ui five statistics">
					...
					<div class="statistic">
						<div class="value"><h2 class="f k r">${fifa.record.stat.summaxgoalfor}</h2></div>
						<div class="label"><h5 class="f k r">득점</h5></div>
					</div>
					<div class="statistic">
						<div class="value"><h2 class="f k r">${fifa.record.stat.summaxgoalagainst}</h2></div>
						<div class="label"><h5 class="f k r">실점</h5></div>
					</div>
					<div class="statistic">
						<div class="value"><h2 class="f k r">${fifa.record.stat.summaxgoalfor - fifa.record.stat.summaxgoalagainst}</h2></div>
						<div class="label"><h5 class="f k r">득실차</h5></div>
					</div>
					<div class="statistic">
						<div class="value"><h2 class="f k r">${fifa.record.stat.summaxpoint}</h2></div>
					...
					<c:forEach var="stat" items="${fifa.record.statseason}">
						<tr>
							<td style="width:20%; border-right:1px solid rgba(34,36,38,.1)">
								<a class="link f k r" href="${path}/league/review.ll?season=${stat.season}">
									${stat.season}/${f:substring(stat.season+1, 2, 4)}
								</a>
							</td>
							<td style="width:55%; border-right:1px solid rgba(34,36,38,.1)">
								<a class="link f k r" href="${path}/fifa/profile.ll?role=Club&name=${stat.club}">
									<img class="ui avatar image" src="${path}/image.ll?role=Club&name=${stat.club}">
									${stat.club}
								</a>
							</td>
				...
				<tbody>
					<c:forEach var="stat" items="${fifa.record.statseason}">
						<tr>
							...
							<td style="width:5%">${stat.maxgame}</td>
							<td style="width:5%">${stat.maxwin}</td>
							<td style="width:5%">${stat.maxdraw}</td>
							<td style="width:5%">${stat.maxlose}</td>
							<td style="width:5%">${stat.maxpoint}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
