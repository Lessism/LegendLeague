...
<section class="ui black segment container">
	...
	<div class="ui grid">
		<div class="ui seven wide column">
			...
			<form method="post" action="${path}/edit/roster_league.ll" id="form_roster">
				<table class="ui striped center aligned table">
					...
					<tbody class="roster">
						<c:forEach var="roster" items="${rosterlist.roster}">
							<tr>
								<td style="width:10%"><input type="checkbox" class="removechk"></td>
								<td style="width:20%"><img class="ui rounded fluid image" src="${path}/image.ll?no=${roster.emblem}"></td>
								<td class="addedclub" style="width:70%"><input type="hidden" name="rosterlist" value="${roster.name}">${roster.name}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
		...
		<div class="ui seven wide column">
			...
			<table class="ui striped center aligned table">
				...
				<tbody>
				<c:forEach var="club" items="${rosterlist.list}">
					<tr>
						<td style="width:10%"><input type="checkbox" class="addchk"></td>
						<td style="width:10%" class="clubemblem"><img class="ui rounded fluid image" src="${path}/image.ll?no=${club.emblem}"></td>
		...
		$('.link.icon').click(function(){
			if ($(this).attr('id') == 'add'){
				for (i = 0; i < $('.addchk:checked').length; i++){
					var clubname = $('.addchk:checked').eq(i).parent().siblings('.clubname').text()
					var clubemblem = $('.addchk:checked').eq(i).parent().siblings('.clubemblem').children('img').attr('src')
					var chk = true
					if ($('.addedclub').length < 10){
						...
						if (chk){
							$('.roster').append(
								'<tr>'+
								'	<td style="width:10%"><input type="checkbox" class="removechk"></td>'+
								'	<td style="width:20%"><img class="ui rounded fluid image" src="'+clubemblem+'"></td>'+
			...
			if ($(this).attr('id') == 'remove'){
				$('.removechk:checked').parent().parent().remove()
			}
		})
	})
