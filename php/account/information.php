...
<?php if (empty($_SESSION['login'])) : ?>
  <section class="ui black segment text container">
  	<form class="ui form" action="<?=$path?>/db/account/login.php" method="post">
	...
	<div class="ui grid black segment container">
		<div class="five wide column" style="padding:50px">
			<div class="ui center aligned card segment" style="max-width:250px; border-top:5px solid #1b1c1d;">
				...
				<div>
          <?php
          $imgNo = ($_SESSION['role'] == 'Club' ? $account['emblem'] : $account['profile']);
          $imgClass = 'ui centered rounded main image';
          ...
          ?>
				...
				<div class="ui country meta f k r">
					<i id="countryicon" class="<?=strtolower($account['country'])?> flag"></i>
					<span><?=$account['country']?></span>
				</div>
				<div class="ui sub meta f k r">
          <?php
            if ($_SESSION['role'] == 'Club') {
              $imgNo = $account['std_img'];
            } else {
              $imgRole = 'Club';
              $imgName = $account['club'];
            }
            $imgClass = 'ui avatar sub image';
            $imgStyle = 'max-width:20px; max-height:20px;';
            include '../db/image.php';
          ?>
					<span><?=$_SESSION['role'] == 'Club' ? $account['stadium'] : $account['club']?></span>
				</div>
				<div class="description">
					<div class="ui active inverted <?=$_SESSION['role'] == 'Player' ? $account['icon'] : 'black'?> progress" style="margin-top:10px; margin-bottom:0;">
						<div class="bar" id="ovrbar" style="width:<?=$account['ovr']+1?>%;">
							<div class="progress f k r" style="color:<?=$_SESSION['role'] == 'Player' ? 'black' : ''?>">
								<span class="position"><?=$_SESSION['role'] == 'Player' ? $account['position'] : 'OVR'?></span>
								<i class="futbol icon"></i>
								<span class="ovr"><?=$account['ovr']?></span>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="eleven wide column" style="padding:50px">
			<form class="ui form" method="post" action="<?=$path?>/db/account/information.php" enctype="multipart/form-data">
						...
						<div class="ui img field">
							<div class="ui labeled input">
								<label class="ui basic label llab f k r" for="image">
									<?=$_SESSION['role'] == 'Club' ? '엠블럼' : '프로필'?>
								</label>
								<div class="ui action input">
									<input class="filetext" type="text" placeholder="Image" readonly>
									<input type="file" accept="image/*" name="img" id="image">
									<input type="hidden" name="img_no" id="img_no" value="<?=$_SESSION['role'] == 'Club' ? $account['emblem'] : $account['profile']?>">
            ...
            <?php if ($_SESSION['role'] == 'Club') : ?>
  						<div class="ui stadium field">
  							<div class="ui labeled input field">
  								...
  								<input type="text" name="stadium" id="stadium" value="<?=$account['stadium']?>">
  								<input type="hidden" name="std_name" id="std_name" value="<?=$account['stadium']?>">
  							</div>
  						</div>
  						<div class="ui stadium_img field">
  							<div class="ui labeled input">
  								...
  								<div class="ui action input">
  									...
  									<input type="hidden" name="std_img" id="std_img" value="<?=$account['std_img']?>">
            ...
            <?php else : ?>
              <div class="ui birth field">
                <div class="ui labeled icon input calendar">
                  ...
                  <input type="text" name="birth" id="birth" autocomplete=off value="<?=$account['birth']?>">
              ...
              <div class="ui height field">
                <div class="ui labeled input">
                  ...
                  <input type="text" name="height" id="height" maxlength="3" value="<?=$account['height']?>">
                </div>
              </div>
              <div class="ui weight field">
                <div class="ui labeled input">
                  <label class="ui basic label llab f k r" for="weight">몸무게</label>
                  <input type="text" name="weight" id="weight" maxlength="3" value="<?=$account['weight']?>">
                </div>
              </div>
              ...
              <div class="ui ovr field">
                <div class="ui labeled input">
                  ...
                  <input type="text" name="ovr" id="ovr" maxlength="2" value="<?=$account['ovr']?>">
                </div>
                <div class="ui float input">
                  <input type="range" id="ovrrange" min="1" max="99" value="<?=$account['ovr']?>">
                </div>
              </div>
            <?php endif; ?>
				</div>
...
$(function(){
  ...
  $('.ui.fluid.search.selection.dropdown').dropdown('set selected', '<?=$account['country']?>')

  $('.tactic.dropdown').dropdown('set selected', '<?=!empty($account['tactic']) ? $account['tactic'] : '4-3-3'?>')

  $('.position.dropdown').dropdown('set selected', '<?=!empty($account['position']) ? $account['position'] : ''?>')
